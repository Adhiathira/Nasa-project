...
class MockDataGenerator:
    ...
    def get_related_papers(self, paper_id: str, conversation: Optional[str] = None, count: int = 5) -> List[Dict[str, Any]]:
        ...
        Returns a selective subset of related papers with variety:
        - Prefers papers with different indices (avoids adjacent papers)
        - Mixes same-topic and cross-topic papers
        - Shuffles results for variety across multiple expand calls
        ...
        source_index = source_paper["index"]

        # Collect candidate papers separately
        same_topic_papers = []
        cross_topic_papers = []

        # Get papers from same topic (exclude source + prefer non-adjacent papers)
        for pid, paper_data in self.paper_database.items():
            if pid != paper_id and paper_data["topic"] == source_topic:
                # Prefer papers with different indices (not immediate neighbors)
                index_distance = abs(paper_data["index"] - source_index)
                if index_distance > 1:  # Skip adjacent papers for more variety
                    same_topic_papers.append(paper_data)
                elif len(same_topic_papers) < count:  # Include adjacent only if needed
                    same_topic_papers.append(paper_data)
        ...
        for topic in related_topics:
            for pid, paper_data in self.paper_database.items():
                if paper_data["topic"] == topic:
                    cross_topic_papers.append(paper_data)

        # Shuffle both lists to add variety across multiple expand calls
        random.shuffle(same_topic_papers)
        random.shuffle(cross_topic_papers)

        # Mix same-topic and cross-topic papers for diversity
        # For count=7: take ~4 same-topic, ~3 cross-topic
        same_count = min(len(same_topic_papers), max(3, (count // 2) + 1))
        cross_count = min(len(cross_topic_papers), count - same_count)

        # Combine selections
        related_papers = same_topic_papers[:same_count] + cross_topic_papers[:cross_count]

        # Final shuffle for variety
        random.shuffle(related_papers)
